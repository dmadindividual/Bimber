      ...
      <div className="flex flex-col lg:flex-row gap-8 w-full mt-12">
        ...
        <div className="flex justify-between bg-inherit items-center  border-b border-gray-700 pb-2 mb-2">
          ...
          {displayTrendingCoins2.map((coin, index) => (
            <Link to={`/coin/${coin.item.id}`} key={index} className="flex  bg-inherit justify-between items-center py-2 border-b border-gray-800 last:border-none">
            <div className="flex items-center ">
            <img src={coin.item.small} alt={coin.item.name} className="w-10 h-10 rounded-full  bg-inherit mr-4" />
            <h4 className="text-base  bg-inherit font-medium">{coin.item.name}</h4>
            </div>
              <h4 className={`text-base font-medium   bg-inherit ${getColor(coin.item.price_btc, coin.item.previous_price_btc)}`}>
                {`${Currency.symbol}${coin.item.price_btc.toFixed(8)}`}
              </h4>
            </Link>
          ))}
        ...
        <div className="flex flex-col gap-4 flex-grow w-full lg:w-96 bg-[#1f2233] p-6 rounded-lg shadow-lg">
          <div className="flex justify-between  bg-inherit items-center border-b border-gray-700 pb-2 mb-2">
          <h3 className="text-2xl font-medium italic  bg-inherit">Overview</h3>
          <h3 className="text-sm text-blue-400  bg-inherit cursor-pointer"><FaArrowRight/></h3>
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-between  bg-inherit flex-1">
